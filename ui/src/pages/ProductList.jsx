...
const ProductList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const categoryRes = await axios.get("http://localhost:3003/categories");
        const categoryRaw = categoryRes.data;

        const categoryData = Array.isArray(categoryRaw)
          ? categoryRaw
          : Array.isArray(categoryRaw?.data)
          ? categoryRaw.data
          : [];

        const nameMap = {};

        // Fill map with main categories and subcategories
        categoryData.forEach((cat) => {
          const mainId = cat._id || cat.id;
          nameMap[mainId] = cat.name;

          if (Array.isArray(cat.subcategories)) {
            cat.subcategories.forEach((subcat) => {
              const subId = subcat._id || subcat.id;
              nameMap[subId] = subcat.name;
            });
          }
        });
          ...
          categoryData.map(async (cat) => {
            try {
              const res = await axios.get(
                `http://localhost:3005/products/by-category?categoryId=${cat._id}`
              );
              const productsInCategory = res.data || [];
              productData.push(...productsInCategory);

              // Group products by category
              const categoryName = nameMap[cat._id] || "Uncategorized";
              if (!grouped[categoryName]) grouped[categoryName] = [];
              grouped[categoryName].push(...productsInCategory);
            } catch (err) {
              console.warn(`Failed to fetch products for category ${cat._id}:`, err.message);
            }
          })
        );
            ...
            {items.slice(0, 8).map((product) => (
              ...
              >
                <div className="relative w-full h-40 overflow-x-auto flex snap-x snap-mandatory">
                  ...
                  product.images.some((img) => img && typeof img === "string") ? (
